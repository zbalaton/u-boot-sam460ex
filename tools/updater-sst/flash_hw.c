...
// ATTENTION ! If the size of struct flash_info increase, padding size need to increase too
...
int flash_print_info(flash_info_t * info)
{
	...
	if ((info->flash_id & FLASH_TYPEMASK) != FLASH_SST040)
	{
...
ulong flash_get_size(vu_long * addr, flash_info_t * info)
{
	...
	addr2[CONFIG_SYS_FLASH_ADDR0] = (CONFIG_SYS_FLASH_WORD_SIZE) 0xaa; //0x00AA00AA;
	addr2[CONFIG_SYS_FLASH_ADDR1] = (CONFIG_SYS_FLASH_WORD_SIZE) 0x55; //0x00550055;
	addr2[CONFIG_SYS_FLASH_ADDR0] = (CONFIG_SYS_FLASH_WORD_SIZE) 0x90; //0x00900090;
	udelay(1000);
...
int flash_erase(flash_info_t * info, int s_first, int s_last)
{
	...
	if (((info->flash_id & FLASH_VENDMASK) == FLASH_MAN_SST) &&
		((info->flash_id & FLASH_TYPEMASK) == FLASH_SST040)) {
		...
		addr[CONFIG_SYS_FLASH_ADDR0] = (CONFIG_SYS_FLASH_WORD_SIZE) 0x00AA00AA;
		addr[CONFIG_SYS_FLASH_ADDR1] = (CONFIG_SYS_FLASH_WORD_SIZE) 0x00550055;
		addr[CONFIG_SYS_FLASH_ADDR0] = (CONFIG_SYS_FLASH_WORD_SIZE) 0x00100010;

		udelay(150000);	/* wait 150 ms */
	}
...
int write_word(flash_info_t * info, ulong dest, ulong data, ulong *pippo)
{
	...
	++(*pippo);
	if (*pippo >= 8192) { *pippo = 0; putc(219); }
