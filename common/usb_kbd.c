				...
				unsigned char modifier, int pressed)
{
	...
	if (pressed == 1) {
		...
		switch(scancode)
		{
			case 41: usb_kbd_put_queue(data, 5); return 0;	// ESC
			case 40: usb_kbd_put_queue(data, 13); return 0;	// ENTER
			case 88: usb_kbd_put_queue(data, 13); return 0;	// NUMPAD ENTER

			case 79: usb_kbd_put_queue(data, 1); return 0;	// CURSOR RIGHT
			case 80: usb_kbd_put_queue(data, 2); return 0;	// CURSOR LEFT
			case 81: usb_kbd_put_queue(data, 3); return 0;	// CURSOR DOWN
			case 82: usb_kbd_put_queue(data, 4); return 0;	// CURSOR UP

			case 75: usb_kbd_put_queue(data, 4); return 0;	// PG UP
			case 78: usb_kbd_put_queue(data, 3); return 0;	// PG DOWN
...
static int usb_kbd_testc(struct stdio_dev *sdev)
{
	 ...
	 */
	if (0 && (get_timer(kbd_testc_tms) < CONFIG_SYS_HZ))
		return 0;
